...
void jit_rvv_layernorm_f16_fused_kernel_t::generate() {
    ...
    Label sum_loop, sum_done, skip_mean, skip_var, out_loop, out_done;

    // ---- pass 1: accumulate Sx and Sx^2 in f32 ----
    ...
    // ---- finish: reduce, mean, variance, inv-std ----
    vsetvli(reg_tmp, x0, SEW::e32, LMUL::m8);
    ...
    vfmv_f_s(f_meansq, v_sumsq); // Sx^2

    ld(reg_len, reg_param, GET_F16_OFF(len));
    fcvt_s_l(f_len, reg_len);
    fdiv_s(f_mean, f_mean, f_len); // mean
    fdiv_s(f_meansq, f_meansq, f_len); // meansq
    ...
    // var = meansq - mean^2 in f64 (spe-faithful finishing)
    fcvt_d_s(d_a, f_mean);
    fmul_d(d_a, d_a, d_a);
    fcvt_d_s(d_b, f_meansq);
    fsub_d(d_b, d_b, d_a); // var (f64)

    fcvt_s_d(f_var, d_b); // variance out (f32)
    fmax_s(f_var, f_var, f_zero);
    ...
    // ---- pass 2: normalize + affine, write f16 ----
    ld(reg_src, reg_param, GET_F16_OFF(src));
