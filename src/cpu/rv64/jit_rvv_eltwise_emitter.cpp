...
constexpr exp_constants_t exp_constants = {
        -103.9720840454f,
        88.7762626647950f,
        0x4b400000,
        0x3fb8aa3b,
...
jit_rvv_eltwise_emitter_t::jit_rvv_eltwise_emitter_t(jit_generator_t *jit)
    : jit_(jit) {}

void jit_rvv_eltwise_emitter_t::load_f32_bits(
        const FReg &dst, const Reg &tmp, uint32_t bits) {
    jit_->li(tmp, static_cast<int64_t>(bits));
    jit_->fmv_w_x(dst, tmp);
}

void jit_rvv_eltwise_emitter_t::load_f32_const(
        const FReg &dst, const Reg &tmp, float value) {
    load_f32_bits(dst, tmp, float_to_bits(value));
}

void jit_rvv_eltwise_emitter_t::broadcast_f32_bits(
        const eltwise_regs_t &r, const VReg &dst, uint32_t bits) {
    load_f32_bits(r.ftmp0, r.tmp_reg0, bits);
    jit_->vfmv_v_f(dst, r.ftmp0);
}

void jit_rvv_eltwise_emitter_t::broadcast_f32_const(
        const eltwise_regs_t &r, const VReg &dst, float value) {
    broadcast_f32_bits(r, dst, float_to_bits(value));
}

void jit_rvv_eltwise_emitter_t::clamp_f32(
        const eltwise_regs_t &r, const VReg &dst, float lower, float upper) {
...
void jit_rvv_eltwise_emitter_t::mul_f32_const(const eltwise_regs_t &r,
        const VReg &dst, const VReg &src, float value) {
    load_f32_const(r.ftmp0, r.tmp_reg0, value);
    jit_->vfmul_vf(dst, src, r.ftmp0);
}

void jit_rvv_eltwise_emitter_t::horner_f32(const eltwise_regs_t &r,
...
void jit_rvv_eltwise_emitter_t::horner_f32_bits(const eltwise_regs_t &r,
...
void jit_rvv_eltwise_emitter_t::abs(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    UNUSED(r);
    jit_->vfabs_v(dst, src);
}

void jit_rvv_eltwise_emitter_t::clip(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    jit_->vfmax_vf(dst, src, r.alpha);
    jit_->vfmin_vf(dst, dst, r.beta);
}

void jit_rvv_eltwise_emitter_t::hardsigmoid(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
...
void jit_rvv_eltwise_emitter_t::hardswish(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
...
void jit_rvv_eltwise_emitter_t::linear(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    jit_->vfmul_vf(dst, src, r.alpha);
    jit_->vfadd_vf(dst, dst, r.beta);
}

void jit_rvv_eltwise_emitter_t::relu(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    const VReg v_mask(0);
...
void jit_rvv_eltwise_emitter_t::sqrt(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    UNUSED(r);
    jit_->vfsqrt_v(dst, src);
}

void jit_rvv_eltwise_emitter_t::square(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    UNUSED(r);
    jit_->vfmul_vv(dst, src, src);
}

void jit_rvv_eltwise_emitter_t::leakyrelu(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
...
void jit_rvv_eltwise_emitter_t::round(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    jit_->vfcvt_x_f_v(r.tmp0, src);
    jit_->vfcvt_f_x_v(dst, r.tmp0);
}

void jit_rvv_eltwise_emitter_t::exp_with_constants(const eltwise_regs_t &r,
        const VReg &dst, const VReg &src, const exp_constants_t &c) {
    jit_->vmv_v_v(dst, src);
    clamp_f32(r, dst, c.LowerRange, c.UpperRange);

    broadcast_f32_bits(r, r.tmp0, c.RoundingBias);
    load_f32_bits(r.ftmp0, r.tmp_reg0, c.Log2Reciprocal);
    jit_->vfmacc_vf(r.tmp0, r.ftmp0, dst);

    load_f32_bits(r.ftmp0, r.tmp_reg0, c.RoundingBias);
    jit_->vfsub_vf(r.tmp1, r.tmp0, r.ftmp0);
    load_f32_bits(r.ftmp0, r.tmp_reg0, c.Log2High);
    jit_->vfmacc_vf(dst, r.ftmp0, r.tmp1);
    load_f32_bits(r.ftmp0, r.tmp_reg0, c.Log2Low);
    jit_->vfmacc_vf(dst, r.ftmp0, r.tmp1);

    horner_f32_bits(r, r.tmp2, dst, exp_poly, array_size(exp_poly));

    jit_->vsll_vi(r.tmp0, r.tmp0, 23);
    jit_->li(r.tmp_reg0, c.MaximumExponent);
    jit_->li(r.tmp_reg1, c.MinimumExponent);
...
void jit_rvv_eltwise_emitter_t::exp(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    exp_with_constants(r, dst, src, exp_constants);
}

void jit_rvv_eltwise_emitter_t::erf_exp_complement(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
...
void jit_rvv_eltwise_emitter_t::tanh(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    jit_->vmv_v_v(dst, src);
...
void jit_rvv_eltwise_emitter_t::erf(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    const VReg v_mask(0);
...
void jit_rvv_eltwise_emitter_t::reciprocal(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
...
void jit_rvv_eltwise_emitter_t::sigmoid(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    const VReg v_mask(0);
    ...
    exp_with_constants(r, r.tmp3, r.tmp3, exp_constants);
...
void jit_rvv_eltwise_emitter_t::swish(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    jit_->vfmul_vf(r.tmp0, src, r.alpha);
...
void jit_rvv_eltwise_emitter_t::elu(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
    const VReg v_mask(0);
    exp_with_constants(r, r.tmp3, src, exp_constants);
...
void jit_rvv_eltwise_emitter_t::gelu_tanh(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
...
void jit_rvv_eltwise_emitter_t::gelu_erf(
        const eltwise_regs_t &r, const VReg &dst, const VReg &src) {
